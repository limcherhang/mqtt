...
class MQTT():
    ...
    def __on_message_raw(self, client, userdata, message):
        logger.debug(f"{message.payload.decode('utf-8')} on {message.topic} in __on_message_raw func.")
        msg = str(message.payload.decode('utf-8'))
        logger.info(f"Receiving Message: '{msg}' from {self.broker} on topic {message.topic}")
        self.q.put(msg)
