...
class MQTT():
    ...
    def __on_message_pm(self, client, userdata, message):
        ...
        logging.info(f"topic split: {message.topic.split('/')}")
    ...
    def __on_message_flow(self, client, userdata, message):###################################################################### 試試看直接把未解碼的message回傳試試看
...
if __name__ == '__main__':
    ...
    logging.basicConfig(
        handlers=[TimedRotatingFileHandler(f'./log/MQTT_SubPPSSRaw.log', when='midnight')], 
        level = loggingLevel, 
