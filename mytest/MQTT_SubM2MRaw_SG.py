...
logger = _logger.get_logger('./log/MQTT_SubM2MRaw_SG.log')
...
logger.info(f"Subscriber Topic: '{subTopic_rawdata}'")
logger.info("===============================================================================================================")

dataQueue = queue.Queue(maxsize=0)

for ip in MQTTips:
    subMQTT = MQTT_M2M(ip, MQTTport, MQTTuser, MQTTpwd, subClient_rawdata, subTopic_list, dataQueue)
    subMQTT.conn()
...
while True:
    ...
    if value_string != '':
        ...
        logger.info("The following value is going to insert to rawData.BACnet!")
