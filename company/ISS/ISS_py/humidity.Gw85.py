...
for rows in name:
    with conn.cursor() as cursor:
        #sqlCommand =f""""REPLACE INTO dataPlatform.humidity (ts, siteId, name, humidity) SELECT 
        sqlCommand =f"""SELECT 
                        ts,
                        '85',
                        '{rows}',
                        ...
                        ts >= '{st}' and 
                        ts < '{et}' and data->'$.humidity' is not NULL;
                    """
        ...
        for row in cursor:
            ...
            name = row[2]
            humidity = row[3]
            value_string+=f"('{ts}',85,'{name}',{humidity}), "

if value_string != '':
    value_string=value_string[:-2]
    with conn2.cursor() as cursor2:
        sql2 = f" REPLACE INTO dataPlatform.humidity (ts, siteId, name, humidity) Values {value_string}"
