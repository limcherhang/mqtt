...
for rows in name:
    with conn.cursor() as cursor:
        #sqlCommand =f""""REPLACE INTO dataPlatform.lightLevel (ts, siteId, name, lightLevel) SELECT 
        sqlCommand =f"""SELECT
                        ts,
                        '65',
                        '{rows}',
                        ...
                        ts >= '{st}' and 
                        ts < '{et}' and data->'$.light_level' is not NULL;
                    """
        ...
        for row in cursor:
                ...
                name = row[2]
                lightlevel = row[3]
                value_string+=f"('{ts}',65,'{name}',{lightlevel}), "

if value_string != '':
    value_string=value_string[:-2]
    with conn2.cursor() as cursor2:
        sql2 = f" REPLACE INTO dataPlatform.lightLevel (ts, siteId, name, lightlevel) Values {value_string}"
