...
class MQTT():
    ...
    def conn(self):
        ...
        #self.client.on_message = self.__on_message_raw # [('m2m/+/raw', 2), ('M2M/gatewayID/raw', 2)]
        self.client.message_callback_add('m2m/+/raw', self.__on_message_raw) 
        #self.client.message_callback_add('M2M/gatewayID/raw', self.__on_message_HDB) # 'M2M/gatewayID/raw'
        #self.client.message_callback_add('+/065166', self.__on_message_JHP)
        try:
    ...
    def __on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            self.client.subscribe(self.topic, qos=2) # [('m2m/+/raw', 2), ('M2M/gatewayID/raw', 2)]
        elif rc == 1:
    ...
    def __on_message_HDB(self, client, userdata, message):
        logging.debug(f"{message.payload.decode('utf-8')} on {message.topic} in __on_message_HDB func.")
        msg = str(message.payload.decode('utf-8'))
        logging.info(f"Receiving Message: '{msg}' from {self.broker} on topic {message.topic}")
        self.q.put(msg)
        print(message.topic)
    ...
    #eric
    def __on_message_JHP(self, client, userdata, message):
        logging.debug(f"{message.payload.decode('utf-8')} on {message.topic} in __on_message_HDB func.")
        msg = str(message.payload.decode('utf-8'))
        logging.info(f"Receiving Message: '{msg}' from {self.broker} on topic {message.topic}")
        self.q.put(msg)
...
if __name__ == '__main__':
    ...
    try:
        ...
        #subTopic_rawdata_Hank = 'Singrow/065165' # Hank's MQTT Topic: Singrow/065165 for M2M GW in Singrow
        #subTopic_rawdata_HDB = 'M2M/gatewayID/raw' # HDB Smart Pump Testing MQTT Topic: 'M2M/gatewayID/raw'
        #eric test
        #subTopic_rawdata_JHP = '+/065166'
        #
        #subTopic_list = [(subTopic_rawdata, 2), (subTopic_rawdata_HDB, 2),(subTopic_rawdata_JHP,2)]
    ...
    logging.basicConfig(
        handlers=[TimedRotatingFileHandler(f'./log/MQTT_SubM2MRaw_SG.log', when='midnight')], 
        level = loggingLevel, 
    ...
    #logging.info(f"Subscriber Topic: '{subTopic_rawdata}' & '{subTopic_rawdata_HDB}'")
    ...
    while True:
        ...
        for data in data_list:
            if mysql_reconnected_flag:
                with open(f'fail_Message_{datetime.datetime.now().date()}.txt', 'a') as msg_f:
                    msg_f.write(data + '\n')
            else:
                dict = json.loads(data)
                ts = parser.parse(dict['T']).astimezone(pytz.timezone('Asia/Singapore')).strftime('%Y-%m-%d %H:%M:%S')
                for d in dict['DATA']:
        ...
        if value_string != '':
            ...
            sqlCommand = f"insert into `rawData`.`M2M` (`DBts`, `GWts`, `gatewayId`, `name`, `rawdata`) Values {value_string}"
            try:
