...
localFileSave = "mbdata.txt"
tempFile = "mbtmp.txt"

topic = "rawdata/modbus/raw"
clientId = "SUBmodbusraw"
logLevel = ""
...
logging.basicConfig(
    handlers=[RotatingFileHandler('log/modbusraw.log', maxBytes=1000000, backupCount=10)],
...
class varInit:
	def __init__(self):
		...
		self.sqlmodbusraw = "INSERT INTO rawData.Modbus (DBts, GWts, gatewayId, cmd, rawdata) VALUES "
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        if(reconnect):
            reconnect = False
            logging.info("MQTT submodbusraw reconnected.")
    else:
...
if(__name__ == "__main__"):
    #out = check_output("ps aux | grep MQSUB_mbraw.py | grep -v 'grep' | awk '{print $2}'", shell=True).decode().split('\n')
    #if(len(out) > 2):
                ...
                if(len(lines) > 0 and len(lines) <= 100):
                    ...
                    while(len(lines)):
                        ...
                        if(rawTemp[:6] == "INSERT"):
                            ...
                            sql = f"INSERT INTO rawData.Modbus (DBts, GWts, gatewayId, cmd, rawdata) VALUES ({raw});"
                    ...
                    for i in range(len(lines)):
                        raw = lines[i][:len(lines[i])-1]
                        
                        if(raw[:6] == "INSERT"):
                            raw = "'" + str(datetime.now()) + "'," + raw.split("VALUES")[1][1:]
                            var.sqlClockIn += f"({raw}),"
                        else:
                            var.sqlmodbusraw += f"({raw}),"
                        ...
                        if(i != 0 and i % 1000 == 0):
                            ...
                            try:
                                if(var.sqlmodbusraw[-2:-1] == ")"):
                                    cursor.execute(var.sqlmodbusraw[:-1])
                                    conn.commit()
                            except Exception as e:
