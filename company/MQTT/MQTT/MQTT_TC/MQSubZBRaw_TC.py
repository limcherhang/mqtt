...
localFileSave = "zbdata.txt"
tempFile = "zbtmp.txt"

topic = "rawdata/zb/raw"
clientId = "SUBzbraw"
logLevel = ""
...
logging.basicConfig(
    handlers=[RotatingFileHandler('log/zbraw.log', maxBytes=1000000, backupCount=10)],
...
class varInit:
	def __init__(self):
		...
		self.sqlzbraw = "INSERT INTO rawData.ZB (`DBts`, `GWts`, `ZBts`, `gatewayId`, `ieee`, `clusterID`, `rawdata`) VALUES "
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        if(reconnect):
            reconnect = False
            logging.info("MQTT subzbraw reconnected.")
    else:
...
if(__name__ == "__main__"):
    #out = check_output("ps aux | grep MQSUB_zbraw.py | grep -v 'grep' | awk '{print $2}'", shell=True).decode().split('\n')
    #if(len(out) > 2):
            ...
            if os.path.isfile(localFileSave):
                ...
                if(len(lines) > 0 and len(lines) <= 100):
                    ...
                    while(len(lines)):
                        ...
                        sql = f"INSERT INTO rawData.ZB (`DBts`, `GWts`, `ZBts`, `gatewayId`, `ieee`, `clusterID`, `rawdata`) VALUES ({raw});"
