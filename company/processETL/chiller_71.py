...
def getPowerData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select powerConsumed from dataPlatform.power where ts='{ts}' and siteId=71 and name='{name}'"
...
def getPlantPowerData(conn, ts):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select round(sum(powerConsumed), 2) from dataPlatform.power where ts='{ts}' and siteId=71 and name!='power#10' and powerConsumed>1"
        logging.debug(sqlCommand)
...
def getTempData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select temp from dataPlatform.temp where ts='{ts}' and siteId=71 and name='{name}'"
        logging.debug(sqlCommand)
...
def getFlowData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select flowRate from dataPlatform.flow where ts='{ts}' and siteId=71 and name='{name}'"
        logging.debug(sqlCommand)
...
def insertTemp(conn, ts, name, temp):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`temp` (`ts`, `siteId`, `name`, `temp`) Values ('{ts}', 71, '{name}', {temp})"
...
def insertPower(conn, ts, name, powerConsumed):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`power` (`ts`, `siteId`, `name`, `powerConsumed`) Values ('{ts}', 71, '{name}', {powerConsumed})"
        logging.debug(replace_sql)
...
def main():
    ...
    while st <= et:
        ...
        if ch1_power is None or ch2_power is None:
            logging.debug(f"{ch1_power}, {ch2_power}")
            continue
        ...
        for index, name in enumerate(chiller_list):
            ...
            if index != chiller_list.index('chiller#plant'):
                ...
                insertPower(conn, ts, 'power#10', plant_powerData)
                ...
                if plant_opFlag is None:
                    ...
                    insertTemp(conn, ts, 'temp#9', plant_supplyTempData)
                    insertTemp(conn, ts, 'temp#10', plant_returnTempData)
                ...
                logging.info(f"'{ts}', 71, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
                value_string += f"('{ts}', 71, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
                continue
            ...
            logging.info(f"'{ts}', 71, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
            value_string += f"('{ts}', 71, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
