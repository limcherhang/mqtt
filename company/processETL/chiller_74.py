...
def getPowerData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select powerConsumed from dataPlatform.power where ts='{ts}' and siteId=74 and name='{name}'"
...
def getPlantPowerData(conn, ts):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select round(sum(powerConsumed), 2) from dataPlatform.power where ts='{ts}' and siteId=74 and name!='power#17' and powerConsumed>1"
        logging.debug(sqlCommand)
...
def getTempData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select temp from dataPlatform.temp where ts='{ts}' and siteId=74 and name='{name}'"
        logging.debug(sqlCommand)
...
def getFlowData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select flowRate from dataPlatform.flow where ts='{ts}' and siteId=74 and name='{name}'"
        logging.debug(sqlCommand)
...
def insertTemp(conn, ts, name, temp):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`temp` (`ts`, `siteId`, `name`, `temp`) Values ('{ts}', 74, '{name}', {temp})"
...
def insertPower(conn, ts, name, powerConsumed):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`power` (`ts`, `siteId`, `name`, `powerConsumed`) Values ('{ts}', 74, '{name}', {powerConsumed})"
        logging.debug(replace_sql)
...
def main():
    ...
    chiller_list = ['chiller#1', 'chiller#2', 'chiller#3', 'chiller#4', 'chiller#plant']
    power_list = ['power#1', 'power#2', 'power#3', 'power#4']
    chp_power_list = ['power#9', 'power#10', 'power#11', 'power#12']
    cdp_power_list = ['power#13', 'power#14', 'power#15', 'power#16']
    ct_power_list = ['power#5', 'power#6', 'power#7', 'power#8']
    supplyTemp_list = ['temp#2', 'temp#6', 'temp#10', 'temp#14']
    returnTemp_list = ['temp#1', 'temp#5', 'temp#9', 'temp#13']
    hdrFlow = 'flow#1'
    ...
    while st <= et:
        ...
        if ch1_power is None or ch2_power is None or ch3_power is None or ch4_power is None or plant_flowrateData is None:
            logging.debug(f"{ch1_power}, {ch2_power}, {ch3_power}, {plant_flowrateData}")
            continue
        else:
            total_power = ch1_power + ch2_power + ch3_power + ch4_power
        ...
        for index, name in enumerate(chiller_list):
            ...
            if index != chiller_list.index('chiller#plant'):
                ...
                insertPower(conn, ts, 'power#17', plant_powerData)
                ...
                if plant_opFlag is None:
                    ...
                    insertTemp(conn, ts, 'temp#17', plant_supplyTempData)
                    insertTemp(conn, ts, 'temp#18', plant_returnTempData)
                ...
                logging.info(f"'{ts}', 74, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
                value_string += f"('{ts}', 74, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
                continue
            ...
            logging.info(f"'{ts}', 74, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
            value_string += f"('{ts}', 74, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
