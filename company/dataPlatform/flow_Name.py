...
where gatewayId>0 and protocol='Name' and dataETLName like 'Flow#%'
"""
cursor.execute(sqlCommand)
cnt=0
for rows in cursor:
    ...
    for data in cursor:
        print(f"({n+1}) {data}")
        ts=data[0]
        flowRate=('NULL' if data[1] is None else round(data[1], 2))
        ...
        with conn.cursor() as replace_cursor:
            ...
            '{ts}', {sId}, '{name}', {flowRate}, {liquidLevel}, {waterConsumed}, {flowTotalPositive}
            )
            """
            print(replace_sql)
