...
def main():
    ...
    month = date_from[5:7] #nowTime.month
    year = date_from[0:4] #nowTime.year
    ...
    for rows in cursor:
        ...
        with conn.cursor() as data_cursor:
            sqlCommand = f"SELECT total FROM dataPlatform{year}.power_{month} where ts>='{date_from}' and siteId={sId} and name='{name}' order by ts asc limit 1"
        ...
        with conn.cursor() as data_cursor:
            sqlCommand = f"SELECT total FROM dataPlatform{year}.power_{month} where ts<'{date_to}' and siteId={sId} and name='{name}' order by ts desc limit 1"
        ...
        logging.info(f"{int(month)}, '{(nowTime-timedelta(days=1)).strftime('%Y-%m-%d')}', {sId}, '{name}', {energyConsumption}, {total}")
        value_string += f"({int(month)}, '{(nowTime-timedelta(days=1)).strftime('%Y-%m-%d')}', {sId}, '{name}', {energyConsumption}, {total}), "
