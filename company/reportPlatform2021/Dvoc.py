...
sqlCommand = f"SELECT siteId, name FROM mgmtETL.NameList where tableDesc='voc' and gatewayId>0 and protocol is Not NULL"
...
for rows in my_cursor:
    ...
    with my_conn.cursor() as data_cursor:
        if history_flag:
            sqlCommand = f"select VOC from dataPlatform{st.year}.voc_{st.month:02} where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
        else:
            sqlCommand = f"select VOC from dataPlatform.voc where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
...
if value_string != '':
    ...
    with my_conn.cursor() as my_cursor:
        replace_sql = f"replace into `reportPlatform{st.year}`.`Dvoc` (`date`, `siteId`, `name`, `vocMin`, `voc25th`, `vocMedian`, `voc75th`, `vocMax`) Values {value_string}"
        try:
