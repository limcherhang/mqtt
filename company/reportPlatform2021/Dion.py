...
sqlCommand = f"SELECT siteId, name FROM mgmtETL.NameList where tableDesc='ion' and gatewayId>0 and protocol is Not NULL"
...
for rows in my_cursor:
    ...
    with my_conn.cursor() as data_cursor:
        if history_flag:
            sqlCommand = f"select density from dataPlatform{st.year}.ion_{st.month:02} where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
        else:
            sqlCommand = f"select density from dataPlatform.ion where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
    ...
    if len(data_list) != 0:
        densityMin = round(np.percentile(np.array(data_list), 0), 2)
        density25th = round(np.percentile(np.array(data_list), 25), 2)
        densityMedian = round(np.percentile(np.array(data_list), 50), 2)
        density75th = round(np.percentile(np.array(data_list), 75), 2)
        densityMax = round(np.percentile(np.array(data_list), 100), 2)
    
    value_string += f"('{date}', {sId}, '{name}', {densityMin}, {density25th}, {densityMedian}, {density75th}, {densityMax}), "

if value_string != '':
    value_string = value_string[:-2]
    with my_conn.cursor() as my_cursor:
        replace_sql = f"replace into `reportPlatform{st.year}`.`Dion` (`date`, `siteId`, `name`, `densityMin`, `density25th`, `densityMedian`, `density75th`, `densityMax`) Values {value_string}"
        try:
