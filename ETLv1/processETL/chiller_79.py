...
def getPowerData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select powerConsumed from dataPlatform.power where ts='{ts}' and siteId=79 and name='{name}'"
...
def getPlantPowerData(conn, ts):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select round(sum(powerConsumed), 2) from dataPlatform.power where ts='{ts}' and siteId=79 and name!='power#9' and powerConsumed>1"
        logging.debug(sqlCommand)
...
def getTempData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select temp from dataPlatform.temp where ts='{ts}' and siteId=79 and name='{name}'"
        logging.debug(sqlCommand)
...
def getFlowData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select flowRate from dataPlatform.flow where ts='{ts}' and siteId=79 and name='{name}'"
        logging.debug(sqlCommand)
...
def insertTemp(conn, ts, name, temp):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`temp` (`ts`, `siteId`, `name`, `temp`) Values ('{ts}', 79, '{name}', {temp})"
...
def insertPower(conn, ts, name, powerConsumed):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`power` (`ts`, `siteId`, `name`, `powerConsumed`) Values ('{ts}', 79, '{name}', {powerConsumed})"
        logging.debug(replace_sql)
...
def main():
    ...
    chiller_list = ['chiller#1', 'chiller#2', 'chiller#plant']
    power_list = ['power#3', 'power#2']
    chp_power_list = ['power#1', 'power#6']
    cdp_power_list = ['power#5', 'power#8']
    ct_power_list = ['power#7', 'power#4']
    supplyTemp_list = ['temp#2', 'temp#6']
    returnTemp_list = ['temp#1', 'temp#5']
    hdrFlow = 'flow#2'
    ...
    while st <= et:
        ...
        if ch1_power is None or ch2_power is None or plant_flowrateData is None:
            logging.debug(f"{ch1_power}, {ch2_power}, {plant_flowrateData}")
            continue
        else:
        ...
        for index, name in enumerate(chiller_list):
            ...
            if index != chiller_list.index('chiller#plant'):
                ...
                insertPower(conn, ts, 'power#9', plant_powerData)
                ...
                if plant_opFlag is None:
                    ...
                    insertTemp(conn, ts, 'temp#10', plant_supplyTempData)
                    insertTemp(conn, ts, 'temp#9', plant_returnTempData)
                ...
                logging.info(f"'{ts}', 79, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
                value_string += f"('{ts}', 79, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
                continue
            ...
            logging.info(f"'{ts}', 79, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
            value_string += f"('{ts}', 79, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
