...
def getPowerData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select powerConsumed from dataPlatform.power where ts='{ts}' and siteId=73 and name='{name}'"
...
def getPlantPowerData(conn, ts):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select round(sum(powerConsumed), 2) from dataPlatform.power where ts='{ts}' and siteId=73 and name!='power#11' and powerConsumed>1"
        logging.debug(sqlCommand)
...
def getTempData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select temp from dataPlatform.temp where ts='{ts}' and siteId=73 and name='{name}'"
        logging.debug(sqlCommand)
...
def getFlowData(conn, ts, name):
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select flowRate from dataPlatform.flow where ts='{ts}' and siteId=73 and name='{name}'"
        logging.debug(sqlCommand)
...
def insertTemp(conn, ts, name, temp):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`temp` (`ts`, `siteId`, `name`, `temp`) Values ('{ts}', 73, '{name}', {temp})"
...
def insertPower(conn, ts, name, powerConsumed):
    ...
    with conn.cursor() as cursor:
        replace_sql = f"replace into `dataPlatform`.`power` (`ts`, `siteId`, `name`, `powerConsumed`) Values ('{ts}', 73, '{name}', {powerConsumed})"
        logging.debug(replace_sql)
...
def main():
    ...
    power_list = ['power#1', 'power#2']
    supplyTemp_list = ['temp#2', 'temp#5']
    returnTemp_list = ['temp#1', 'temp#6']
    hdrFlow = 'flow#1'
    ...
    while st <= et:
        ...
        for index, name in enumerate(chiller_list):
            ...
            if index != chiller_list.index('chiller#plant'):
                ...
                insertPower(conn, ts, 'power#11', plant_powerData)
                ...
                logging.info(f"'{ts}', 73, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
                value_string += f"('{ts}', 73, '{name}', {plant_opFlag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
                continue
            ...
            logging.info(f"'{ts}', 73, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}")
            value_string += f"('{ts}', 73, '{name}', {op_Flag}, {round(coolingCapacityData, 3)}, {round(efficiencyData, 3)}), "
