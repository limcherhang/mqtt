...
def main():
    ...
    #nowTime = datetime.now().replace(day=5,hour= 15,microsecond=0)
    #st = (nowTime - timedelta(minutes=40)).replace(second=0)
    #et = (nowTime - timedelta(minutes=20)).replace(second=0)
    st = (nowTime - timedelta(minutes=20)).replace(second=0)
    et = nowTime
    logging.info(f"---- Searching from {st} to {et} ----")
    value_string = ''
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,deviceId,name FROM mgmtETL.sigfoxAPI where deviceId = '496961'"
        cursor.execute(sql)
        for row in cursor:
            gId = row[0]
            dId = row[1]
            ...
            with conn.cursor() as data_cursor:
                ...
                if data_cursor.rowcount == 0:
                    logging.info(f'Power#3 has no data')
                    continue
                else:
