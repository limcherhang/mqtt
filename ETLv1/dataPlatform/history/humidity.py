...
# et = nowTime
st = datetime(2023, 5, 13, 14, 6, 0)
print ("Time st :", st)
et = datetime(2023, 5, 15, 10, 18, 0)
print ("Time et :", et)
...
sqlCommand = f"select siteId, name, tableDesc, gatewayId, protocol, dataETLName, dataETLValue from mgmtETL.NameList where tableDesc='humidity' and siteId=65 and gatewayId>0 and protocol is NOT NULL"
...
if value_string != '':
    ...
    with my_conn.cursor() as my_cursor:
        replace_sql = f"replace into `dataPlatform2023`.`humidity_05` (`ts`, `siteId`, `name`, `humidity`) Values {value_string}"
        try:
