...
def getTotal(conn, gId, name, st, et):
    with conn.cursor() as cursor:
        #sqlCommand=f"select totalPositiveWattHour from dataETL2023.power_05 where gatewayId={gId} and name='{name}' and ts>='{st}' and ts<'{et}' order by ts asc limit 1"
        sqlCommand=f"select totalPositiveWattHour from dataETL2023.power_04 where gatewayId={gId} and name='{name}' and ts>='{stTotal}' and ts<'{et}' order by ts asc limit 1"
        #print(sqlCommand)
...
def chillerPlant(conn, sId, name, gId, st, et):
    ...
    while st<et:
        ...
        with conn.cursor() as cursor:
            if sId == 24:
                sqlCommand=f"select sum(if(ch1Watt is NULL, 0, ch1Watt)), sum(if(ch2Watt is NULL, 0, ch2Watt)), sum(if(ch3Watt is NULL, 0, ch3Watt)), sum(if(powerConsumed is NULL, 0, powerConsumed)) from dataPlatform2023.power_05 where siteId={sId} and name in ({power_string[:-2]}) and ts='{ts}'"
                cursor.execute(sqlCommand)
                data=cursor.fetchone()
        ...
        with conn.cursor() as replace_cursor:
            replace_sql=f"""
            replace into `dataPlatform2023`.`power_05`(
            `ts`, `siteId`, `name`, 
...
#st = nowTime.strftime('%Y-%m-%d 0:0:0') 
#st = (nowTime-timedelta(minutes=828)).replace(second=0)
#st = nowTime.strftime('%Y-%m-13 00:00:00') 
st = datetime(2023, 4, 3, 0, 0, 0)
stTotal = datetime(2023, 4, 3, 0, 0, 0)
print ("Time st :", st)
et = datetime(2023, 4, 4, 0, 0, 0)
...
#exit()
#conn=connectDB('192.168.6.50')
conn=connectDB('127.0.0.1')
#conn=connectDB('192.168.1.62')

site_cursor = conn.cursor()
sqlCommand="select siteId, name, gatewayId, protocol, dataETLName from mgmtETL.NameList where tableDesc='power' and gatewayId>0 and siteId=10 and protocol is NOT NULL order by siteId, convert(substring_index(name, '#', -1), unsigned integer)"
...
for rows in site_cursor:
    ...
    cursor = conn.cursor()
    sqlCommand=f"select ts, ch1Watt, ch2Watt, ch3Watt, totalPositiveWattHour, ch1Current, ch2Current, ch3Current from dataETL2023.power_04 where gatewayId={gId} and name='{ETLName}' and ts>='{st}' and ts<'{et}'"
    #print(sqlCommand)
    cursor.execute(sqlCommand)
    if cursor.rowcount==0:
...
print(f"----- Connnection Closed ----- took: {(programEndTime-programStartTime).seconds}s")