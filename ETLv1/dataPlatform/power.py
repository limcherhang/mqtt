...
sqlCommand="select siteId, name, gatewayId, protocol, dataETLName from mgmtETL.NameList where tableDesc='power' and siteId!=87 and gatewayId>0 and protocol is NOT NULL order by siteId, convert(substring_index(name, '#', -1), unsigned integer)"
...
for rows in site_cursor:
    ...
    if protocol=='Combination' and ETLName is None:
        chillerPlant(conn, sId, name, gId, st, et)
        cnt+=1
        continue
