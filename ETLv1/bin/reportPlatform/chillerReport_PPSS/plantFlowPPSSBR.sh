...
if [ "${1}" == "" ] || [ "${2}" == "" ] || [ "${3}" == "" ] || [ "${4}" == "" ] || [ "${5}" == "" ]; then
        echo "請輸入 2020-01-09 109 00124b000be4cbb8 ultrasonicFlow2"
		echo "		Start Day"
		echo "		Gateway ID"
		echo "		chiller Flow IEEE"
		echo "		chiller Table"
		echo "      Main or PAHU"
        exit 1
fi
...
if [ ${1} == $today ]; then
	...
	flowTable=${4}_$dbdataMonth
	#flowCoolingTable=${9}_$dbdataMonth
...
if [ $flowIEEE == 0 ] || [ $flowTable == 0 ] ; then
	...
	echo "  flowIEEE $flowIEEE flowTable:$flowTable $name"
	#echo "  flowCoolingIEEE $flowCoolingIEEE flowCoolingTable:$flowCoolingTable"
fi
...
mysql -h ${host} -D$dbRPF -ss -e"REPlACE INTO dailyPlantFlow(operationDate,siteId,gatewayId,name,
	chillerFlowRateMin,chillerFlowRateMedian,chillerFlowRateMax,
	CoolingFlowRateMin,CoolingFlowRateMedian,CoolingFlowRateMax,
	chillerFlowRateData,
	CoolingFlowRateData) 
