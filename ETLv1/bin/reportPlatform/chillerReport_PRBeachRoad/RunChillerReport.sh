...
echo "  ----------------DataBase--------------------- "
echo "  dbdata $dbdata Table_$dbdataMonth siteId:$siteId"
...
do
	...
	mysql -h ${host} -D$dbRPF -ss -e"REPlACE INTO dailyDataQuality (operationDate,siteId,gatewayId,deviceDescription,idealDataCount,
						  actualDataCount,reportRate
						 ) 
						VALUES ('$startDay', '$siteId', '$gId', '$deviceDesc','$dataCount', '${qualityData[0]}', '$dataRate');
						"
...
if [ $chilledWaterFlowHDRNum == 1 ]; then
	...
	echo "  $flowHDRDeviceDesc $flowHDRTable $flowHDRIEEE '$startDay' '$siteId' '$gId' '$deviceDesc' '$dataCount' '${qualityData[0]}' '$dataRate'"
...
do
	...
	echo "  ${DeviceInfoBuf[2]} ${flowTable[$arrNum]} ${flowIEEE[$arrNum]} '$startDay' '$siteId' '$gId' '$deviceDesc' '$dataCount' '${qualityData[0]}' '$dataRate'"
...
do
	...
	echo "  $deviceDesc $deviceIEEE $deviceTable '$startDay' '$siteId' '$gId' '$deviceDesc' '$dataCount' '${qualityData[0]}' '$dataRate'"
...
FROM iotmgmtChiller.vDeviceInfo 
where 
	gatewayId=128 and deviceDesc like 'CoolingTower#%'
order by deviceNum asc
;
"))
	...
	if [ $historyTable == 1 ]; then
		qualityData=($(mysql -h ${host} -D$dbdata -ss -e"SELECT count(*) as tol,count(*)/1440 as per
					...
					from
						...
						SELECT *
						FROM ${deviceTable}_$dbdataMonth WHERE ieee='$deviceIEEE' and
						gatewayId=128 and receivedSync >='$startDay $startTime' and receivedSync <'$endDay 01:00'
						) as a 
					WHERE receivedSync >='$startDay $startTime' and receivedSync <'$endDay $endTime'
					GROUP BY time
					)as a
