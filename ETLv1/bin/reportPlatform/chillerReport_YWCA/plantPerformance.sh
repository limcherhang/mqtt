...
nameList=("power#25" "power#26" "power#27" "power#28" "power#29" "power#30")
chillerNameList=("NULL" "power#19" "power#20")

totalEnergyConsumption=0 #kwh

whileNum=0
while :
do

	if [ "${nameList[$whileNum]}" == "" ]; then
		break
	fi
	
	totalEnergyConsumption=($(mysql -h ${host} -D$dbRPF2021 -ss -e"
	select
		round($totalEnergyConsumption+IFNULL(total,0),3)
	FROM
		 Dpower
	WHERE name='${nameList[$whileNum]}' and
		  date ='$startDay' and
		  siteId = $siteId
	"))

	echo "[DEBUG]${nameList[$whileNum]} $totalEnergyConsumption "
	
	whileNum=$(($whileNum+1))
done


totalEnergyConsumption=($(mysql -h ${host} -D$dbRPF -ss -e"
	select
		round($totalEnergyConsumption+(IFNULL(sum(totalPowerWh),0)/1000),3)
	FROM
		 dailyChillerData
	WHERE operationFlag = 1 and
...
pumpTotal=($(mysql -h ${host} -D$dbRPF -ss -e"SELECT 
            sum(IFNULL(totalPowerWh,0))/1000
		FROM 
		    reportplatform.dailyPumpData
		WHERE 
		    operationDate='$startDay' and 
...
if [ "$pumpTotal" != "NULL" ]; then
	data=($(mysql -h ${host} -D$dbRPF -ss -e"SELECT 
          $pumpTotal+Round(((Round(sum(IFNULL(totalPowerWh,0))/1000,2))/$totalEnergyConsumptionNum)*100,2) as kWh 
		FROM 
		    reportplatform.dailyCoolingPumpData
		WHERE 
		    operationDate='$startDay' and 
