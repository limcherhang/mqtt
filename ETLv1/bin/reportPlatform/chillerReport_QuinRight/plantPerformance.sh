...
do
	...
	if [ -f "./buf/kW.$startDay.$gId.$whileHour" ]; then
		...
		jsonNum=$(($jsonNum+1))
					  #>=
		if [ $jsonNum -ge 2 ]; then
			printf ",">> ./buf/kW.Json.$startDay.$gId
		fi
...
do
	...
	if [ -f "./buf/coolingCapacity.$startDay.$gId.$whileHour" ]; then
		...
		jsonNum=$(($jsonNum+1))
					  #>=
		if [ $jsonNum -ge 2 ]; then
			printf ",">> ./buf/coolingCapacity.Json.$startDay.$gId
		fi
...
do
	...
	if [ -f "./buf/efficiency.$startDay.$gId.$whileHour" ]; then
		...
		jsonNum=$(($jsonNum+1))
					  #>=
		if [ $jsonNum -ge 2 ]; then
			printf ",">> ./buf/efficiency.Json.$startDay.$gId
		fi
...
# powerMax=($(mysql -h ${host} -D$dbProcess -ss -e"
		# SELECT 
			# efficiency
		# FROM 
		# (
...
# powerMin=($(mysql -h ${host} -D$dbProcess -ss -e"
		# SELECT 
			# efficiency
		# FROM 
		# (
...
# powerMedian=($(mysql -h ${host} -D$dbProcess -ss -e"
		# SELECT 
			# efficiency
		# FROM 
		# (
