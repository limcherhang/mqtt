...
if [ "${1}" == "" ] || [ "${2}" == "" ] || [ "${3}" == "" ] || [ "${4}" == "" ] || [ "${5}" == "" ] || [ "${6}" == "" ] || [ "${7}" == "" ] || [ "${8}" == "" ] || [ "${9}" == "" ]; then
        echo "bash ./plantCombinedFlow.sh 2020-09-24 00:00 2020-09-25 00:00 109 00124b000be4cbfd ultrasonicFlow2 00124b000be4cb73 ultrasonicFlow2"
		echo "		2020-01-09"
		echo "		00:00"
		echo "		2020-01-09"
		echo "		00:00"
		echo "		Gateway ID"
		echo "		chiller Flow1 IEEE"
		echo "		chiller Table1"
		echo "		chiller Flow2 IEEE"
		echo "		chiller Table2"
        exit 1
fi
...
if [ ${1} == $today ]; then
	...
	flowTable1=${7}_$dbdataMonth
	flowTable2=${9}_$dbdataMonth
...
do
	...
	do
		...
		if [ $flowTrue == 1 ]; then
			...
			flowData=($(mysql -h ${host} -D$dbdata -ss -e"
			SELECT truncate(flowRate,2) as flowRate
			 FROM (
							Select a.syncTime as ts1,b.syncTime as ts2,a.flowRate+b.flowRate as flowRate
							from
							(
							 SELECT date_format(receivedSync, '%Y-%m-%d %H:%i') syncTime,Round(flowRate,2) as flowRate FROM $flowTable1
								WHERE 
								  ieee='$flowIEEE1' and 
							...
							as a
							 left join
							(
							 SELECT date_format(receivedSync, '%Y-%m-%d %H:%i') syncTime,Round(flowRate,2) as flowRate FROM $flowTable2
								WHERE 
								  ieee = '$flowIEEE2' and 
								  receivedSync>='$startDay $stHour:$stMin' and 
								  receivedSync<='$startDay $endHour:$endMin' and 
				 ...
				 GROUP BY date_format(ts1, '%Y-%m-%d %H:%i');
			"))
			if [ "$flowData" != "" ]; then
			 echo "$flowData" >> ./buf/flowDataHDR.$startDay.$gId
			 echo "$flowData" >> ./buf/flowDataHDR.$startDay.$gId.$stHour
			else
			 echo " $stHour:$stMin ~ $endHour:$endMin || $runStartHour $runStartMin $runEndHour $runEndMin $flowData"
			fi
			# CoolingFlowData=($(mysql -h ${host} -D$dbdata -ss -e"
			# SELECT truncate(flowRate,2)
			 # FROM 
			   # $flowCoolingTable
			 # WHERE ieee='$flowCoolingIEEE' and
				# receivedSync >= '$startDay $stHour:$stMin' and 
				# receivedSync < '$endDay $endHour:$endMin' and 
...
do
	...
	if [ -f "./buf/flowDataHDR.$startDay.$gId.$whileHour" ]; then
		...
		do
			...
			flowData="$(cat ./buf/flowDataHDR.$startDay.$gId.$whileHour | head -n $calNum | tail -n 1)"
			
			#echo "$dataTotalFlow+$flowData"
			echo "scale=3;$dataTotalFlow+$flowData"|bc > ./buf/dataTotalFlow.$startDay.$gId
