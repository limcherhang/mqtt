...
echo "bash ./coolingPump.sh $IEEE $startDay $startTime $endDay $endTime $capacityValue 1000"
bash ./coolingPump.sh $IEEE $startDay $startTime $endDay $endTime $capacityValue 1000

if [ ! -f "./data/coolingPump.$startDay.$IEEE" ]; then
	#檔案不存在
    echo "[ERROR]Directory ./data/coolingPump.$startDay.$IEEE does not exists."
	exit 0
fi

time_num="$(cat ./data/coolingPump.$startDay.$IEEE | head -n 1 | tail -n 1)"

if [ $time_num == 0 ]; then
	echo "++++ cooling pump is no data ++++"
	...
	mysql -h ${host} -D$dbRPF -ss -e"REPlACE INTO dailyCoolingPumpData(
		operationDate,siteId,gatewayId,pumpId,pumpDescription,startTime,endTime,
		operationMinutes,dataCount
		) 
		VALUES(
...
do
	...
	#start time
	start_day="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $start_day_head | tail -n 1)"
	start_time="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $start_time_head | tail -n 1)"
	
	#end time
	end_day="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $end_day_head | tail -n 1)"
	end_time="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $end_time_head | tail -n 1)"

	#operationFlag
	flag="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $flag_head | tail -n 1)"
	
	#total kw
	powerWh="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $powerWh_head | tail -n 1)"

	#runMinutes
	runMinutes="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $runMinutes_head | tail -n 1)"
	
	#Count
	dataCount="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $count_head | tail -n 1)"	
	
	#Power Consumption
	powerConsumption="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $powerConsumption_head | tail -n 1)"
	
	#Power Consumption Level
	for i in {0..5};
	do
		powerConsumptionNum=$(($powerConsumptionLevel_head+$i))
		powerPer[i]="$(cat ./data/coolingPump.$startDay.$IEEE | head -n $powerConsumptionNum | tail -n 1)"
	done
...
rm ./data/coolingPump.$startDay.$IEEE
exit 0