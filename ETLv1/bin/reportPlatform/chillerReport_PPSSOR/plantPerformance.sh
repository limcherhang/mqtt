...
echo "  Cooling Water Flow HDR Num:$chilledWaterFlowHDRNum"
...
if [ $CoolingWaterFlowHDRNum == 1 ]; then
	...
	if [ $historyTable == 1 ]; then
		coolingflowHDRTable=${DeviceInfoBuf[1]}_$dbdataMonth
	else
	...
	echo "  $coolingflowHDRDeviceDesc $coolingflowHDRIEEE $coolingflowHDRTable"
fi
...
powerData=($(mysql -h ${host} -D$dbdata -ss -e"
   ...
   FROM $pmTable 
   where 
       ieee in ('${chillerIEEE[0]}','${chillerIEEE[1]}',
				'${pumpIEEE[0]}','${pumpIEEE[1]}',
	            '${coolingIEEE[0]}','${coolingIEEE[1]}',
				'${coolingWaterPumpIEEE[0]}','${coolingWaterPumpIEEE[1]}') and 
...
if [ -f "./buf/energyConsumptionDataJson.$startDay.$gId" ]; then
	...
	rm ./buf/energyConsumptionDataJson.$startDay.$gId
	# echo "[DEBUG] energyConsumptionData :$energyConsumptionData"
else
