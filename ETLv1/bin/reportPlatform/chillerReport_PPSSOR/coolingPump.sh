...
if [ "${data[$dataNum]}" == '' ]; then
	echo "++++$IEEE $startDay $startTime~$endDay $endTime Cooling Water Pump Power Meter no data+++"
	echo "0" > ./data/coolingPump.$startDay.$IEEE
	exit 0
fi
...
echo "$arrNum" > ./data/coolingPump.$startDay.$IEEE
...
do
	...
	#start time
	echo "${startDayArr[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE
	echo "${startTimeArr[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE
	
	#end time
	echo "${endDayArr[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE
	echo "${endTimeArr[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE
	
	#operationFlag
	#echo "operationFlag:${flag[$arrNum]}"
	echo "${flag[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE

	#total kw
	totalkwh=($(mysql -h ${host} -D$db -ss -e"select sum(totalWattHour)
	from
	(
	...
	if [ "$totalkwh" == "NULL" ]; then
		echo "0" >> ./data/coolingPump.$startDay.$IEEE
	else
		echo "$totalkwh" >> ./data/coolingPump.$startDay.$IEEE
	fi
	...
	echo "$runMinutes" >> ./data/coolingPump.$startDay.$IEEE
	
	#dataCount
	#echo "count=${count[$arrNum]}"
	echo "${count[$arrNum]}" >> ./data/coolingPump.$startDay.$IEEE
	...
	do
		...
		#Power Consumption
		echo "${powerMeterData[$whileNum]}" >> ./buf/coolingPump.$startDay.$IEEE
		whileNum=$(($whileNum+1))
	...
	if [ ! -f "./buf/coolingPump.$startDay.$IEEE" ]; then
		#檔案不存在
		echo "[ERROR]Directory ./buf/coolingPump.$startDay.$IEEE does not exists."
		
		#Power Consumption(Median)
		echo "$powerConsumption" >> ./data/coolingPump.$startDay.$IEEE
		
		#Power Consumption Level
		echo "$powerConsumptionLevel_0" >> ./data/coolingPump.$startDay.$IEEE
		echo "$powerConsumptionLevel_1" >> ./data/coolingPump.$startDay.$IEEE 
		echo "$powerConsumptionLevel_2" >> ./data/coolingPump.$startDay.$IEEE 
		echo "$powerConsumptionLevel_3" >> ./data/coolingPump.$startDay.$IEEE 
		echo "$powerConsumptionLevel_4" >> ./data/coolingPump.$startDay.$IEEE 
		echo "$powerConsumptionLevel_5" >> ./data/coolingPump.$startDay.$IEEE
		
	else
	
		#Power Consumption
		sort -n ./buf/coolingPump.$startDay.$IEEE >> ./buf/coolingPump.$startDay.$IEEE.sort
		powerConsumption="$(cat ./buf/coolingPump.$startDay.$IEEE.sort | head -n $medianNum | tail -n 1)"

		rm ./buf/coolingPump.$startDay.$IEEE
		rm ./buf/coolingPump.$startDay.$IEEE.sort
