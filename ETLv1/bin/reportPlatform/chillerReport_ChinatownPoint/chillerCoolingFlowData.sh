...
chillerTimeData=($(mysql -h ${host} -D$reportPlatform -ss -e"select 
  ...
  FROM dailyChillerData
	  ...
	  operationDate ='$startDay' and operationFlag=1
"))
...
do
	...
	do
		...
		echo "flow Data:${flowData[$whileNum]}"
		whileNum=$(($whileNum+1))
	done
	...
	echo "replace INTO dailyCoolingWaterFlow(operationDate,siteId,gatewayId,chillerId,chillerDescription,flowMin,flowMedian,flowMax,
			flowData
		  ) 
		...
		"
	mysql -h ${host} -D$reportPlatform -ss -e"replace INTO dailyCoolingWaterFlow(operationDate,siteId,gatewayId,chillerId,chillerDescription,flowMin,flowMedian,flowMax,
			flowData
		  ) 
		VALUES('$startDay','$siteId','$gatewayId','$chillerNum', '$chillerName',
			if($flowMin is NULL,NULL,'$flowMin'),
