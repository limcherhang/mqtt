...
sqlCommand = f"select siteId, name from mgmtETL.NameList where gatewayId>0 and protocol is not NULL and tableDesc='noise'"
...
for rows in rd_cursor:
    ...
    with rd_conn.cursor() as data_cursor:
        sqlCommand = f"select dB from dataPlatform.noise where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
    ...
    if len(data_list) != 0:
        dBMin = round(np.percentile(np.array(data_list), 0) ,2)
        dBMedian = round(np.percentile(np.array(data_list), 50) ,2)
        dBMax = round(np.percentile(np.array(data_list), 100) ,2)

        value_string += f"('{date}', {sId}, '{name}', {dBMin}, {dBMedian}, {dBMax}), "

if value_string != '':
    value_string = value_string[:-2]
    with rd_conn.cursor() as replace_cursor:
        replace_sql = f"replace into `reportPlatform{year}`.`Dnoise` (`date`, `siteId`, `name`, `dBMin`, `dBMedian`, `dBMax`) Values {value_string}"
        try:
