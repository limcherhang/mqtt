...
sqlCommand = f"SELECT siteId, name FROM mgmtETL.NameList where tableDesc='ch2o' and gatewayId>0 and protocol is Not NULL"
...
for rows in my_cursor:
    ...
    with my_conn.cursor() as data_cursor:
        if history_flag:
            sqlCommand = f"select CH2O from dataPlatform{st.year}.ch2o_{st.month:02} where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
        else:
            sqlCommand = f"select CH2O from dataPlatform.ch2o where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
...
if value_string != '':
    ...
    with my_conn.cursor() as my_cursor:
        replace_sql = f"replace into `reportPlatform{st.year}`.`Dch2o` (`date`, `siteId`, `name`, `ch2oMin`, `ch2o25th`, `ch2oMedian`, `ch2o75th`, `ch2oMax`) Values {value_string}"
        try:
