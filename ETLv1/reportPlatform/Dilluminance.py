...
sqlCommand = f"select siteId, name from mgmtETL.NameList where gatewayId>0 and protocol is not NULL and tableDesc='illuminance'"
...
for rows in rd_cursor:
    ...
    with rd_conn.cursor() as data_cursor:
        sqlCommand = f"select light from dataPlatform.illuminance where siteId={sId} and name='{name}' and ts>='{st}' and ts<'{et}'"
        ...
        for data in data_cursor:
            ...
            if light is not None:
                data_list.append(light)
    
    if len(data_list) != 0:
        lightMin = round(np.percentile(np.array(data_list), 0) ,2)
        lightMedian = round(np.percentile(np.array(data_list), 50) ,2)
        lightMax = round(np.percentile(np.array(data_list), 100) ,2)

        value_string += f"('{date}', {sId}, '{name}', {lightMin}, {lightMedian}, {lightMax}), "

if value_string != '':
    value_string = value_string[:-2]
    with rd_conn.cursor() as replace_cursor:
        replace_sql = f"replace into `reportPlatform{year}`.`Dilluminance` (`date`, `siteId`, `name`, `lightMin`, `lightMedian`, `lightMax`) Values {value_string}"
        try:
