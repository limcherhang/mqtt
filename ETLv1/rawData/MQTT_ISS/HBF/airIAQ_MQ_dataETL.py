...
def is_connected(conn,sql):
    ...
    with conn.cursor() as cursor:
        try:
            ...
            logging.error(f"[MySQL Insert EEROR]:{str(ex)}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        subTopic_rawdata = 'ISSHBF/iaqsensor'
    except Exception as ex:
        sys.exit(f"[Config Setting ERROR]: {str(ex)}")

    logging.basicConfig(
        handlers=[TimedRotatingFileHandler(f'./log/{__file__}.log', when='D', interval=1, atTime=datetime.time(hour=0, minute=0, second=0))], 
        level = loggingLevel, 
    ...
    while True:
        ...
        if not dataQueue.empty():
            ...
            value_string = f"('{datetime.datetime.now().replace(microsecond=0)}','301','{value_string}'" + ')'
