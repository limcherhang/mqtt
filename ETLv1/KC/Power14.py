...
def main():
    ...
    with conn.cursor() as data_cursor:
        sqlCommand = f"select ts,ch1Watt,totalNegativeWattHour from dataETL.power where ts>='{st}' and ts<'{et}' and  totalNegativeWattHour is not null and name = 'power#1' and gatewayId = 261 order by ts asc"
        ...
        if data_cursor.rowcount == 0:
            ...
            for data in data_cursor:
                ts = data[0]
                ch1watt = (0 if data[1] > 0 else(float(data[1])))
                ...
                value_string += f"('{ts}',87,'power#14',{abs(ch1watt)}, {abs(power)}, {energy}, {nega}), "
