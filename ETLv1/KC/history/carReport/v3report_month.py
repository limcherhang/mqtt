...
def main():
    ...
    nowTime = datetime.now().replace(day=1,hour=0,minute=0,second=0,microsecond=0)
    st = nowTime 
    et = nowTime + relativedelta(months=1)
    ...
    with conn.cursor() as cursor:
        ...
        for rows in cursor:
            ...
            with conn.cursor() as cursor_1:
                ...
                for s in cursor_1:
                    ...
                    stime = st
                    etime = stime 
                    while etime <= (currentdate - timedelta(days=1)):

                        etime = etime + timedelta(days=1)
                        with conn.cursor() as cursor_2:
                            sql_2 = f"SELECT ts,mileage,distance,fuelConsumption,totalFuelConsumption FROM dataPlatform{year}.car where name = '{name}' and ts >= '{stime}' and ts < '{etime}' and siteId = {sId} order by ts desc limit 1"
        ...
        if value_string != '':
            ...
            with conn.cursor() as cursor:
                replace_sql = f"replace into reportPlatform{year}.Dcar(ts,siteId,name,mileage,distance,fuelConsumption,totalFuelConsumption) Values {value_string}"
        ...
        with conn.cursor() as cursor:
            ...
            if value_string_2 != '':
                ...
                with conn.cursor() as cursor_1:
                    replace_sql = f"replace into reportPlatform{year}.Mcar(ts,siteId,name,mileage,distance,fuelConsumption,totalFuelConsumption) Values {value_string_2}"
