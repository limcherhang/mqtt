...
def main():
    ...
    value_string = ''
    with conn.cursor() as cursor:
        sql = f"SELECT gatewayId,name,dataETLName FROM mgmtETL.NameList where siteId = 87 and gatewayId = 261 and dataETLName != 'Power#1' "
        cursor.execute(sql)
        for row in cursor:
            gId = row[0]
            name = row[1]
            ...
            with conn.cursor() as data_cursor:
                sqlCommand = f"select ts,ch1Watt,totalPositiveWattHour from dataETL2023.power_07 where ts>='{st}' and ts<'{et}'  and name = '{etlname}' and gatewayId = '{gId}' order by ts asc"
