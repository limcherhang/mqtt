...
def main():
    ...
    et = nowTime
    current_st = nowTime.replace(hour=0,minute=0,second=0,microsecond=0)
    ...
    for level in range(1,num+1):
        ...
        with conn.cursor() as cursor:
            ...
            for rows in cursor:
                ...
                with conn.cursor() as cursor:
                    sql = f"select ts from dataPlatform.power where siteId = {sId} and name = '{main}' and ts >=  '{st}' and ts <'{et}' order by ts desc limit 1 "
                    cursor.execute(sql)
                    logging.info(sql)
